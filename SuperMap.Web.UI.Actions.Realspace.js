...
SuperMap.Web.UI.Action3Ds.SelectEx.prototype = {
    ...
    */
    onMouseDown: function(e) {
        ///<param name="e" type="EventObject"></param>
        ///<returns type="void"></returns>
        if (e.get_flagType() == (SuperMap.Web.Realspace.FlagType.LBUTTON | SuperMap.Web.Realspace.FlagType.HITGLOBE)) // 左键单击按下
        {
    ...
    */
    onDbClick: function(e) {
        ///<param name="e" type="EventObject"></param> 
        ///<returns type="void"></returns>
        if (e.get_flagType() == SuperMap.Web.Realspace.FlagType.LBUTTON) // 左键2击才去查询和桌面组件一致
        {
            // 使用高亮图片时需要查询
    ...
    */
    onMouseUp: function(e) {
        ...
        if (e.get_flagType() == SuperMap.Web.Realspace.FlagType.LBUTTON) // 左键单击按下
        {
            if (this._isSupportGridQuery()) {
                var pos = new SuperMap.Web.Core.Point2D(e.get_longitude(), e.get_latitude());
                // 处理投影
